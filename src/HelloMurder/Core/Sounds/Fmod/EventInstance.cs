...
namespace HelloMurder.Core.Sounds.Fmod
{
    ...
    public class EventInstance : IDisposable
    {
        ...
        public bool Set3DAttributes(SoundSpatialAttributes attributes)
        {
            FMOD.RESULT result = _instance.Set3DAttributes(attributes.ToFmodAttributes());
            if (result != FMOD.RESULT.OK)
            {
                return false;
            }
        ...
        public bool Resume()
        {
            FMOD.RESULT result = _instance.GetPaused(out bool paused);
            if (result != FMOD.RESULT.OK || !paused)
            {
                return false;
            }

            result = _instance.SetPaused(false);
            if (result != FMOD.RESULT.OK)
            {
                return false;
            }
        ...
        public bool Pause()
        {
            ...
            result = _instance.SetPaused(true);
            if (result != FMOD.RESULT.OK)
            {
                return false;
            }
